...
export default function ItemVenta({venta}: productType) {
    ...
    useEffect(() => {
        setCounter( prev => {
            const found = cartLStorage?.cartItems.find(item => item.id === venta.id);
            if(found) {
    ...
    const handleClickPlus = () => {
        if(counter < 20) {
            setCounter(++counter);
            cartLStorage?.setQuantity(venta.id, counter);
        }
    }
    ...
    const handlePrice = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if(newPrice) {
            setPrice(newPrice);
            cartLStorage?.setPrice(venta.id, newPrice);
        }
    }

    const handleRemove = () => {
        cartLStorage?.removeVenta(venta.id);
    }
